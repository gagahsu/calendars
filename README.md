...
帳單提醒除了文字摘要，還會附上每張卡一個的卡片，上面有「✅ 已繳費 / 未繳費」兩個
按鈕，點一下就標記，不用打字。按鈕帶的是 statement id，所以標記的一定是畫面上那一
期，不會像文字指令那樣需要猜月份。逾期後每天都會再提醒一次，直到標記為止。

繳款日前 3 天若金額還沒登記，就算那張卡的 `remindDaysBefore` 沒有 3 也會提醒，卡片
